...
func create_tables() {
	...
	execSQL("create table if not exists student_statuses (id INT AUTO_INCREMENT NOT NULL, student_id INT, problem_id INT, coding_stat VARCHAR(50), help_stat VARCHAR(50), submission_stat VARCHAR(50), percentage int, explanation TEXT, last_updated_at timestamp, PRIMARY KEY (`id`))")
	...
	execSQL("CREATE TABLE IF NOT EXISTS global_maps ( id INT AUTO_INCREMENT NOT NULL,     teacher_map LONGTEXT NOT NULL,  teacher_pass LONGTEXT NOT NULL,  teacher_name_to_id LONGTEXT NOT NULL,    teacher_id_to_name LONGTEXT NOT NULL,    students LONGTEXT NOT NULL,     bulletin_board LONGTEXT NOT NULL,     working_subs LONGTEXT NOT NULL,    submissions LONGTEXT NOT NULL,    working_help_subs LONGTEXT NOT NULL,  help_submissions LONGTEXT NOT NULL,    active_problems LONGTEXT NOT NULL,    help_eligible_students LONGTEXT NOT NULL,     seen_help_submissions LONGTEXT NOT NULL, snapshots LONGTEXT NOT NULL,   student_snapshot LONGTEXT NOT NULL,   created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,   updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,     PRIMARY KEY (id));")
